...
<head>
    ...
    <title>Dashboard Admin — <?= htmlspecialchars($namaPria) ?> &amp; <?= htmlspecialchars($namaWanita) ?></title>
...
<aside class="sidebar" id="sidebar">
    <div class="sidebar-logo">
        ...
        <div class="couple-names" id="sb-couple"><?= htmlspecialchars($namaPria) ?> &amp; <?= htmlspecialchars($namaWanita) ?></div>
...
async function apiGet(action, extra = '') {
    const res  = await fetch(`api.php?action=${action}${extra}`);
...
function renderTable(list, bodyId) {
    ...
    tbody.innerHTML = list.map((g, i) => {
        ...
        const inviteLink = g.slug ? `${window.location.origin.replace('/undangan/dashboard', '')}/undangan/undangan.php?guest=${encodeURIComponent(g.slug)}` : '';
        const linkDisplay = inviteLink ? `<span class="guest-link">${inviteLink.substring(0,35)}...</span>` : '–';
...
function showPage(pageId, navEl) {
    ...
    const names = { overview:'Overview', tamu:'Daftar Tamu', ucapan:'Ucapan & Doa', statistik:'Statistik RSVP', pengaturan:'Pengaturan' };
